import type { ContentBlock, ResultState, TweetCardModel } from "./types";
import { extractUsernameFromMeta } from "./tweet-card";

const FORMAT_ORDER = ["markdown", "json"] as const;

export type ResultExportFormat = (typeof FORMAT_ORDER)[number];

type ArticleResultState = Extract<ResultState, { kind: "article" }>;
type ThreadResultState = Extract<ResultState, { kind: "thread" }>;
type TweetResultState = Extract<ResultState, { kind: "thread" | "user-tweets" }>;
...
function getTweetRows(result: TweetResultState): TweetRow[] {
...
function toArticleMarkdown(result: ArticleResultState): string {
...
function toTweetMarkdown(result: TweetResultState): string {
...
function toMarkdown(result: ResultState): string {
