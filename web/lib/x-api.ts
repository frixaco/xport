...
function byCreatedAtAsc(a: XPost, b: XPost): number {
  return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime();
}

export function sortTweetsChronologically(tweets: XPost[]): XPost[] {
  return [...tweets].sort(byCreatedAtAsc);
}

export function filterAuthorThread(tweets: XPost[]): XPost[] {
  if (tweets.length === 0) return [];

  const sorted = sortTweetsChronologically(tweets);
  const rootTweet = sorted[0]!;
  const authorId = rootTweet.author.id;
  const threadTweetIds = new Set<string>([rootTweet.id]);
  const result: XPost[] = [rootTweet];

  const repliesByParent = new Map<string, XPost[]>();
  for (const tweet of sorted) {
    if (tweet.inReplyToId) {
      const replies = repliesByParent.get(tweet.inReplyToId) ?? [];
      replies.push(tweet);
      repliesByParent.set(tweet.inReplyToId, replies);
    }
  }
  ...
  while (true) {
    ...
    const authorReply = replies.find((t) => t.author.id === authorId && !visited.has(t.id));

    if (!authorReply) break;

    visited.add(authorReply.id);
    threadTweetIds.add(authorReply.id);
    result.push(authorReply);
    currentId = authorReply.id;
  }
