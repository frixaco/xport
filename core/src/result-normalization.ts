import { normalizeArticleContents } from "./article-content.ts";
import { isNonEmptyString, isObject } from "./payload.ts";
import { parseTweetId, parseUsername } from "./url-parser.ts";
import {
...
import type { RequestType, ResultState, UsageMetadata } from "./types.ts";
...
export function normalizeResult(
  ...
  const authorUsername =
    article && isObject(article.author) && isNonEmptyString(article.author.userName)
      ...
      : null;
  const date = article ? formatDisplayDate(article.createdAt) : null;
...
export function extractErrorMessage(payload: unknown): string | null {
  ...
  if (isNonEmptyString(payload.error_description)) return payload.error_description;
